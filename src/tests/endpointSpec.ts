    ...
    beforeAll(() => {
      ...
      describe('Order Endpoint Testing', ()=>{
        ...
        it('Get current orders for user', async()=>{
            const response = await request.get('/orders/users/1').set('Authorization','Bearer '+ token);
            expect(response.status).toBe(200);
        })

        it('Get current orders for user', async()=>{
            const response = await request.get('/orders/users/1/orders-completed').set('Authorization','Bearer '+ token);
            expect(response.status).toBe(200);
        })
    });
